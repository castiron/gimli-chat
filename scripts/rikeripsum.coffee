# Description:
#   Gimli Riker me, inspired by http://rikeripsum.com/
#
# Commands:
#   
...
module.exports = (robot) ->
	class Riker
		statements: [
			...
			"Earl Grey tea, watercress sandwiches... and Bularian canap̩s? Are you up for promotion?"
		...
		speak: (count) ->
			...
			out = ''
			for i in [1..count]
				out = out + ' ' + @statements[Math.floor(Math.random() * @statements.length)]
			out

	r = new Riker()
	# Can't seem to get whitespace to 
	robot.respond /rikerism (me ?)?(\d*)?/i, (msg) -> msg.send r.speak(msg.match[2]).trim()
	robot.hear /\briker\b/i, (msg) -> msg.send r.speak().trim()

...
F = require '../lib/util/messageFormatter'

module.exports = (robot) ->	
	robot.respond  /(show )?quill( order)?[\s]*$/i, (msg) ->
		...
		if hasOrder
			msg.send F.codeBlock out
		else
			msg.send "There's nothing in the Quill order, mk mk."
	...
	robot.respond /quill remove (\d*)/i, (msg) ->
		key = msg.match[1] - 1
		removeItem key, msg
	...
	robot.respond  /quill (order|add)( me)? (.*)/i, (msg) ->

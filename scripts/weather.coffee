...
#   mbmccormick
#
# Modified to add default location by:
# Naomi Rubin at Cast Iron Coding
env = process.env
...
lookupAddress = (msg, location, cb) ->
  ...
  .get() (err, res, body) ->
    ...
    cb(msg, coords, err, location)

lookupWeather = (msg, coords, err, location) ->
...
parseLocation = (match, defaultLocation='Cast Iron Coding, 97214') ->
  console.log match
  if match == '' || match == ' ' || match == 'me' || match == 'for' || match == 'in'
    defaultLocation
  else
    match
...
module.exports = (robot) ->
  ...
  robot.respond /weather(?: me|for|in)?\s?(.*)/i, (msg) ->
    location = parseLocation(msg.match[1])
